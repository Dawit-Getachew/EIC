...
import { Selectors as InvestmentPermitSelector } from "src/store/States/InvestmentPermit/"
import { selectServiceID, Actions as BufferActions } from "src/store/States/Buffer"
...
function HeaderSearch() {
  ...
  const SearchSuggestion: React.FC<ISearchSuggestion> = ({
    ...
    return (
      <ListItem button onClick={() => {
        dispatch(BufferActions.SetViewPermitBuffer(data))
        navigate(routes.WORK_PERMIT.VIEW_NEW_WORK_PERMIT.ROUTE, { replace: true })
        setOpen(false)
      }}>
        <Hidden smDown>
          <ListItemAvatar>
  ...
  const investment_permits = useSelector(InvestmentPermitSelector.selectInvestmentPermits)
  const service_id = useSelector(selectServiceID)
  const [permits, setPermits] = useState([])

  useEffect(() => {
    setPermits(
      investment_permits.filter(item => String(item.investor_id) === String(service_id))
    )
  }, [investment_permits, service_id])
