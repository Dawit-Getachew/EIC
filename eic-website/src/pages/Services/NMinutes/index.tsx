...
const NMinutesTable = () => {
  ...
  const viewPermit = (investment: any) => {
    ...
    navigate(routes.INVESTMENT.NOTORIZED_MINUTES_FORM.ROUTE, { replace: true })
  }
            ...
            {investment_data.map((investmentPermit, key) => (
              ...
              >
                ...
                <TableCell align="right">
                  <Tooltip title="Notorize Minutes" arrow>
                    <IconButton
                      color="inherit"
                      size="small"
                      onClick={() => viewPermit(investmentPermit)}
                      // disabled={Boolean(String(investmentPermit.draft_minutes).length > 0)}
                    >
