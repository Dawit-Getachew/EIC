...
const DocumentAttach: FC<Props> = (props) => {
  ...
  const handleSubmit = async (data: any) => {
    ...
    if (String(response).length > 10) {
      InvestmentAPI.UpdateMemorandumArticleBankSlip({
        _id: savedBuffer._id,
        memorandum_bank_slip_form: response
      }, (err, data) => {
        if (err) throw err
        if (data._id) {
          setIsLoading(false)
          ...
          <Grid container width="100%" className="form-box-notice">
            ...
            <Grid item md={10}>
              ...
              <Typography variant={"h4"} style={{ fontWeight: "normal" }}>
                Please upload the bank slip for the payment of the memorandum of association change
              </Typography>
            </Grid>
          </Grid>
