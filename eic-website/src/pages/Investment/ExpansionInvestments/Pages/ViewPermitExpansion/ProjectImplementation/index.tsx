  ...
  useEffect(() => {
    if (Object.keys(savedBuffer).length > 0) {
      FormActions.UpdateFormData({
        ...
        ...(() => {
          const plan_keys = Object.keys(savedBuffer.project_impl_plan)
          const plan_values = Object.values(savedBuffer.project_impl_plan)
          let plan_obj = {}
          plan_values.forEach((value: any, idx: number) => {
            if (plan_keys[idx] === "public_utility_acquisition") {
              const utility_keys = Object.keys(value)
              const utility_values = Object.values(value)
              const utility_obj = {}
              utility_values.forEach((item: any, idx: number) => utility_obj[`${utility_keys[idx]}_utility`] = getCommonDate(item))
              plan_obj = {
                ...plan_obj,
                ...utility_obj
              }
            } else {
              plan_obj[plan_keys[idx]] = getCommonDate(value)
            }
          })
          let dateObj = { ...plan_obj }
          if (savedBuffer.starting_date) {
            dateObj = {
              ...dateObj, starting_date: getCommonDate(savedBuffer.starting_date)
            }
          }

          if (savedBuffer.ending_date) {
            dateObj = {
              ...dateObj, ending_date: getCommonDate(savedBuffer.ending_date)
            }
          }
          return dateObj
        })()
          ...
          <Grid item md={4} xs={12}>
            <FormControl className="flex-c" style={{ marginTop: 25 }}>
              ...
              <TextField
                ...
                variant="outlined"
                type="date"
                {...register("project_devt_feasiblility_study", formState, dispatch)}
              />
            </FormControl>
          </Grid>
          <Grid item md={4} xs={12}>
          ...
          <Grid item md={4} xs={12}>
            <FormControl className="flex-c" style={{ marginTop: 25 }}>
              ...
              <TextField
                ...
                variant="outlined"
                type="date"
                {...register("reaching_of_machinery_at_project_site", formState, dispatch)}
              />
            </FormControl>
          </Grid>
          <Grid item md={4} xs={12}>
          ...
          <Grid item md={4} xs={12}>
            <FormControl className="flex-c" style={{ marginTop: 25 }}>
              ...
              <TextField
                ...
                variant="outlined"
                type="date"
                {...register("co_missing_machines_and_make_ready_for_operator", formState, dispatch)}
              />
            </FormControl>
          </Grid>
        </Grid>
