...
const DocumentAttach: FC<Props> = (props) => {
  ...
  const handleSubmit = async (data: any) => {
    ...
    if (String(response).length > 10) {
      InvestmentAPI.UpdateCreditServiceBankSlip({
        _id: savedBuffer._id,
        credit_service_bank_slip_form: response
      }, (err, data) => {
        if (err) throw err
        if (data._id) {
          setIsLoading(false)
  ...
  return (
    <>
      <div className="form-box">
        <div className="form-box-header">Upload your Credit Advice for the Transfer of Minimum Required Capital</div>
        <input
          type="file"
          hidden={true}
          ...
          <Grid container width="100%" className="form-box-notice">
            ...
            <Grid item md={10}>
              ...
              <Typography variant={"h4"} style={{ fontWeight: "normal" }}>
                Please upload the bank slip for the credit service payment
              </Typography>
            </Grid>
          </Grid>
