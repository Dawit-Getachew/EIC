...
const DocumentAttach: FC<Props> = (props) => {
  const upload_types = ["Upload Edited Company Name's Memorandum of Association"];
          ...
          >
            <Grid item md={4} justifyContent="flex-start">
              <div className="flex-r flex-both-start">
                <img
                  src={
                    getFormData(formState, "memorandum_form") ? RightIcon : CloudIcon
                  }
                  className="cloud-icon"
                  alt=""
                />
                <div className="document-attach-h2">Upload Memorandum of Association</div>
              </div>
            </Grid>
            <Grid item md={6}>
  ...
  const handleSubmit = async (data: any) => {
    setIsLoading(true)
    const response = await uploadFile(data.memorandum_form)
    if (String(response).length > 10) {
      InvestmentAPI.UpdateMemorandumArticleForm({
        _id: savedBuffer._id,
  ...
  return (
    <>
      <div className="form-box">
        <div className="form-box-header">Upload Edited Company Name Memorandum of Association</div>
        <input
          type="file"
          hidden={true}
          ...
          <Grid container width="100%" className="form-box-notice">
            ...
            <Grid item md={10}>
              ...
              <Typography variant={"h4"} style={{ fontWeight: "normal" }}>
                Please upload the new and edited memorandum of association
              </Typography>
            </Grid>
          </Grid>
