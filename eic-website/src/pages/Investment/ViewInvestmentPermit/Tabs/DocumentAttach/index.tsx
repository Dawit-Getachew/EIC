...
const DocumentAttach: FC<Props> = (props) => {
          ...
          >
            ...
            <Grid item md={2}>
              <div className="flex-both-center">
                <a href={getFormData(formState, inputs[idx])? String(getFormData(formState, inputs[idx])) : '#'} style={{ textDecoration: "none" }} download target="_blank">
                  <Button
                    style={{
  ...
  useEffect(() => {
    if (Object.keys(savedBuffer).length > 0) {
      FormActions.UpdateFormData(selectRequriedKeys([...inputs, "permit_documents"], savedBuffer), dispatch)
    }
  }, [savedBuffer, dispatch])

  useEffect(() => {
    if (formState.formData.permit_documents) {
      const keys = Object.keys(formState.formData.permit_documents)
      const values = Object.values(formState.formData.permit_documents)
      keys.forEach((key, idx) => {
        FormActions.UpdateFormInput({ name: key, value: values[idx] }, dispatch)
      })
    }
  }, [formState.formData.permit_documents])
