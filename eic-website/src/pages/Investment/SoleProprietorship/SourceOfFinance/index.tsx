...
const SourceOfFinance: FC<Props> = (props) => {
  ...
  const removeProductItemFromRow = (idx: number) => {
    setProductRows(productRows.filter((_, _idx) => idx !== _idx))
    FormActions.UpdateFormInput(
      { name: "products", value: productRows.filter((_, _idx) => idx !== _idx) },
      dispatch
    );
  }

  const removeRawMaterialItemFromRow = (idx: number) => {
    setRawMaterialRows(rawMaterialRows.filter((_, _idx) => idx !== _idx))
    FormActions.UpdateFormInput(
      { name: "raw_materials", value: rawMaterialRows.filter((_, _idx) => idx !== _idx) },
      dispatch
    );
  }
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <IconButton
                            color="error"
                            sx={{ p: 2, mr: 2 }}
                            onClick={() => removeProductItemFromRow(idx)}
                          >
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <IconButton
                            color="error"
                            sx={{ p: 2, mr: 2 }}
                            onClick={() => removeRawMaterialItemFromRow(idx)}
                          >
                            <RemoveCircleOutlineOutlined fontSize="small" />
                          </IconButton>
                        </TableCell>
