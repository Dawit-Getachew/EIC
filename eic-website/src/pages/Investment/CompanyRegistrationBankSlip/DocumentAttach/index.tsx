...
const DocumentAttach: FC<Props> = (props) => {
  ...
  const handleSubmit = async (data: any) => {
    ...
    if (String(response).length > 10) {
      InvestmentAPI.UpdateCompanyRegistrationBankSlipForm({
        _id: savedBuffer._id,
        company_registration_bank_slip_form: response
      }, (err, data) => {
        if (err) throw err
        if (data._id) {
          setIsLoading(false)
  ...
  return (
    <>
      <div className="form-box">
        <div className="form-box-header">Upload Bank Slip</div>
        <input
          type="file"
          hidden={true}
          ...
          <Grid container width="100%" className="form-box-notice">
            ...
            <Grid item md={10}>
              ...
              <Typography variant={"h4"} style={{ fontWeight: "normal" }}>
                Please upload bank slip for the payment of the company registration
              </Typography>
            </Grid>
          </Grid>
