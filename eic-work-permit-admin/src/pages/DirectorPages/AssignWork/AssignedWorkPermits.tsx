...
const AssignedWorkPermits = () => {
  ...
  useEffect(() => {
    ...
    WorkPermitAPI.FetchAssignedWorkPermits((err: any, data: any[]) => {
      if (err) throw err
      if (data) {
  ...
  const assignWorkPermit = (data: any) => {
    if (Object.keys(data).length > 0) {
      ...
      WorkPermitAPI.AssignEmployeeToWorkPermitBody({
        ...
        if (err) throw err
        setFetchedWorkPermits(workPermits => workPermits.map(item => item._id === data._id? data : item))
        setIsLoading(false)
      })
    }
    ...
    <>
      ...
      >
        <Grid item md={4} xs={12}>
          ...
          >
            No Assigned Work Permits for Now
          </Typography>
        </Grid>
      </Grid>
...
export default AssignedWorkPermits