...
import { API, Selectors } from "src/store/States/Investment/Category/SubSector/";
import { Actions as BufferActions } from "src/store/States/Buffer"
import { useDispatch, useSelector } from "react-redux"

const EditSubSector: React.FC<any> = () => {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const selectedSubSector = useSelector(Selectors.selectSelectedSubSector)
  const sectors = useSelector(SectorSelector.selectSectors)

  const onSubmit = (input: ISubSectorInput) => {
    setIsLoading(true);
    API.EditSubSector({
      _id: selectedSubSector._id,
      ...input
    }, (err, data) => {
      if (err) throw err;
      setIsLoading(false);
                ...
                >
                  <Grid item md={6}>
                    <TextField
                      ...
                      defaultValue={selectedSubSector.name}
                    />
                  </Grid>
                  <Grid item md={6}>
                    <TextField
                      ...
                      select
                      defaultValue={selectedSubSector.sector}
                    >
                      {sectors.map(sector => (
