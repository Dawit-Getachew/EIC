  ...
  switch (role) {
    case IRoleAccount.INSPECTOR: {
      return permits.filter(
        (item) =>
          item.permit_status === InvestmentPermitStatus.DRAFTED ||
          item.permit_status === InvestmentPermitStatus.ACCEPTED
      );
    }
    ...
    case IRoleAccount.CASE_WORKER: {
      return permits.filter(
        (item) =>
          item.permit_status === InvestmentPermitStatus.ACCEPTED ||
          item.permit_status === InvestmentPermitStatus.REVIEWED
      );
    }
    ...
    case IRoleAccount.TEAM_LEADER: {
      return permits.filter(
        (item) =>
          item.permit_status === InvestmentPermitStatus.REVIEWED ||
          item.permit_status === InvestmentPermitStatus.VERIFIED
      );
    }
    ...
    case IRoleAccount.DIRECTOR: {
      return permits.filter(
        (item) =>
          item.permit_status === InvestmentPermitStatus.VERIFIED ||
          item.permit_status === InvestmentPermitStatus.CANCELLED
      );
    }
