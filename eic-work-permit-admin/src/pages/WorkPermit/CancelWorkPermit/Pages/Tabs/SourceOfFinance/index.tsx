  ...
  return (
    <>
      <div>
        ...
        />
        <RawMaterialModalElement
          isVisible={isRawMaterialModalVisible}
          onSubmit={addRawMaterialItem}
        />
        <div>
          ...
          <Grid container width="100%" spacing={4}>
            <Grid item md={6} xs={12}>
              ...
              >
                <FormLabel id="demo-radio-buttons-group-label">
                  Estimated of permanent employees at full capacity
                </FormLabel>
                <TextField
                  className="long-input"
                  variant="outlined"
                  type="number"
                  {...register("number_of_employees", formState, dispatch)}
                />
              </FormControl>
            </Grid>
          ...
          >
            <Grid item md={12} xs={12}>
              {getFormError(formState, "market_destination_type").error ? (
                <p style={{ color: "red" }}>
                  {
                    getFormError(formState, "market_destination_type")
                      .helperText
                  }
                </p>
              ) : (
          ...
          >
            <Grid item md={9} xs={12}>
              <div
                className="investment-detail-title-default"
                style={{ marginTop: 20 }}
              >
                Estimated annual production/service rendering of the project at
                full capacity
              </div>
            </Grid>
          </Grid>
          ...
          >
            <Grid item md={9} xs={12}>
              {getFormError(formState, "products").error ? (
                <p style={{ color: "red" }}>
                  {getFormError(formState, "products").helperText}
                </p>
              ) : (
                <></>
              )}
                    ...
                    {productRows.map((row, idx) => (
                      ...
                      >
                        ...
                        <TableCell align="right">
                          {row.local_share_market}
                        </TableCell>
                        <TableCell align="right">
                          {row.export_share_market}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
          ...
          <Grid container width="100%" spacing={4} justifyContent="flex-start">
            <Grid item md={9} xs={12}>
              {getFormError(formState, "raw_materials").error ? (
                <p style={{ color: "red" }}>
                  {getFormError(formState, "raw_materials").helperText}
                </p>
              ) : (
                <></>
              )}
          ...
          >
            <Grid item md={9}>
              <div className="investment-detail-title">
                Breief description about the potential impact of the proposed
                investment on environment and measures of mitigation to be
                undertaken
              </div>
            </Grid>
          </Grid>
          <Grid container width="100%" spacing={4} justifyContent="flex-start">
            <Grid item md={9} xs={12}>
              {getFormError(formState, "enviromental_impact").error ? (
                <p style={{ color: "red" }}>
                  {getFormError(formState, "enviromental_impact").helperText}
                </p>
              ) : (
                <></>
              )}
          ...
          <Grid container width="100%" spacing={4} justifyContent="flex-start">
            <Grid item md={9}>
              <TextareaAutosize
                ...
                {...register("enviromental_impact", formState, dispatch)}
              />
            </Grid>
          </Grid>
        </div>
