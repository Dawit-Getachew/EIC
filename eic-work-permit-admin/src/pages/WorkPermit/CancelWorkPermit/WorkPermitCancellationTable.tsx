...
import CancelIcon from '@mui/icons-material/CancelPresentationTwoTone';
import BulkActions from './BulkActions';
import ViewModal from './ViewModal'
import DeleteModal from './DeleteModal'
import { useDispatch, useSelector } from "react-redux"
import { Actions as WorkPermitCancellationActions, API as WorkPermitCancellationAPI } from "src/store/States/WorkPermit/CancelWorkPermit"
import { useNavigate } from "react-router"
import routes from 'src/constants/routes'
import { filterCancellationPermits, getActionText, getCancelPermitStatus } from "./filter"
...
const RecentOrdersTable: FC<RecentOrdersTableProps> = ({ cryptoOrders }) => {
  const [selectedInvestmentPermitCancellations, setSelectedCryptoOrders] = useState<string[]>(
    []
  );
  ...
  const filteredInvestmentPermitCancellations = applyFilters(cryptoOrders, filters);
  ...
  const [selectedInvestmentPermitCancellation, setSelectedInvestmentPermitCancellation] = useState({})
  ...
  return (
    <Card>
      <ViewModal
        ...
        work_permit={selectedInvestmentPermitCancellation}
      />
      <DeleteModal
      ...
      {!selectedBulkActions && (
        <CardHeader
          ...
          title="Recent Work Permits Cancellation Requests"
        />
      )}
      <Divider />
      <TableContainer>
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell align="right">View Cancellation Form</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {filterCancellationPermits(user_role, paginatedInvestmentPermitCancellations).map((item) => {
              const isCryptoOrderSelected = selectedInvestmentPermitCancellations.includes(
                item._id
              );
              ...
              return (
                ...
                >
                  ...
                  <TableCell align="right">
                    ...
                    <Tooltip title="Delete Permit" arrow>
                      <IconButton
                        ...
                        size="small"
                        onClick={() => {
                          setSelectedInvestmentPermitCancellation(work_permit)
                  ...
                  <TableCell align="right">
                    <Tooltip title="View Cancellation Form" arrow>
                      <IconButton
                        sx={{
                          '&:hover': {
                            background: theme.colors.primary.lighter
                        ...
                        size="small"
                        onClick={() => {
                          dispatch(BufferActions.SetCancellationPermitBuffer(work_permit))
                          navigate(routes.WORK_PERMIT.VIEW_WORK_PERMIT_CANCELLATION.ROUTE, { replace: true })
                        }}
                      >
