...
import { API, Actions } from "src/store/States/Investment/Manager/"
...
const DeleteModal: React.FC<DeleteModalProps> = ({ isVisible, onClose, manager }) => {
  const [isLoading, setIsLoading] = useState(false)
  const dispatch = useDispatch()
  const yesHandler = () => {
    setIsLoading(true)
    API.RemoveManager(manager._id, (err, data) => {
      if (err) throw err
      setIsLoading(false)
      if (data._id) {
        dispatch(Actions.RemoveManager(data))
        onClose()
      }
    })
  }
      ...
      >
        <Box sx={style}>
          <Typography id="modal-modal-title" variant="h4" component="h2" style={{ marginBottom: 15, marginTop: 5 }}>
            Are you sure you want to delete {`${manager.first_name} ${manager.middle_name} ${manager.last_name}`}
            <Grid
              container
              direction="row"
