...
const DeleteModal: React.FC<DeleteModalProps> = ({ isVisible, onClose, investment_permit }) => {
  const [isLoading, setIsLoading] = useState(false)
  const dispatch = useDispatch()
  const yesHandler = () => {
    setIsLoading(true)
    API.RemoveInvestmentPermit(investment_permit._id, (err, data) => {
      if (err) throw err
      setIsLoading(false)
      if (data._id) {
        // dispatch(Actions.RemoveInvestmentPermit(data))
        onClose()
      }
    })
  }
      ...
      >
        <Box sx={style}>
          <Typography id="modal-modal-title" variant="h4" component="h2" style={{ marginBottom: 15, marginTop: 5 }}>
            Are you sure you want to delete {investment_permit.company_name}
            <Grid
              container
              direction="row"
