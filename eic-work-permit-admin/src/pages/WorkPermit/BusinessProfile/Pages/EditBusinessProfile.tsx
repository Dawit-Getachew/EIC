...
import {
  API,
  Selectors as BusinessProfileSelectors,
...
const EditBusinessProfile: React.FC<any> = () => {
  ...
  const business_profile = useSelector(
    BusinessProfileSelectors.selectSelectedBusinessProfile
  );
  ...
  const onSubmit = (input: IBusinessProfileInput) => {
    ...
    API.EditBusinessProfile(
      ...
      (err, data) => {
        ...
        setIsLoading(false);
        if (data._id) {
          navigate(routes.WORK_PERMIT.BUSINESS_PROFILE.ROUTE, {
            replace: true,
          });
        }
      }
