...
import { API as WorkPermitReplacementAPI, Actions as WorkPermitReplacementActions } from './index'
import { Actions as BufferActions, Selectors as BufferSelectors } from 'src/store/States/Buffer'
import { useDispatch, useSelector } from "react-redux"
import DoneIcon from '@mui/icons-material/DoneRounded';

export const UpdateWorkPermitReplacement: FC<{
  user_role: string;
  work_permit: any;
}> = ({
  user_role, work_permit
    ...
    const updateStatus = (_id: string) => {
      setIsLoading(true)
      WorkPermitReplacementAPI.UpdateRepalceWorkPermitStatus({
        _id, permit_status: getReplacePermitStatus(user_role)
      }, (err, data) => {
        if (err) throw err
        if (data._id) {
          dispatch(WorkPermitReplacementActions.UpdateReplaceWorkPermits(data))
          dispatch(BufferActions.SetReplacementPermitBuffer({
            ...savedBuffer,
            ...data
          }))
        }
    ...
    return (
      <Tooltip title={getActionText(user_role)} arrow>
        <IconButton
          ...
          disabled={work_permit.permit_status === getReplacePermitStatus(user_role)}
        >
...
export const getReplacePermitStatus = (role: string) => {
  switch (role) {
    case IRoleAccount.TEAM_LEADER:
...
export const filterReplacementPermits = (role: string, permits: any[]) => {
  switch (role) {
    case IRoleAccount.TEAM_LEADER: {
      return permits.filter(item => (
