...
import { IBasicID, IBasicIDs, IUser, IUserDoc, IPostUser, IEditUser, ISigninUser, IChangePassword, IPostCompany, IChangePhone, IChangeEmail, GQLInputResidenceForm } from "../../Models/User/user.types"
...
export class UserService {
  ...
  static async fetchUserByServiceID(prop: IBasicID) {
    return await (await UserResolver.Query.fetchUserByServiceID({}, prop, {})).data
  }
  ...
  static async updateResidenceUser({ input }: GQLInputResidenceForm) {
    const foundUser = await UserModel.findOne({ service_id: input._id })
    if (!foundUser) {
      throw new Error("User Not Found")
    }

    return await (await UserResolver.Mutation.editUser({}, {
      input: {
        _id: foundUser._id,
        residence_form: {
          letter_of_support: input.letter_of_support,
          visa_image: input.visa_image,
...
/**
 * User Service Module
 * All the functions are subject to the user model
 * 
 */