...
import { GQLInputResidenceForm, IPostUser, ISigninUser } from '../../../Models/User/user.types';
...
const {
  ...
  Mutation: {
    ...
    changeEmail, changePhoneNumber, changePassword, updateResidence
  }
} = UserActions
...
const UserResolver = {
  Query: {
    ...
    async [fetchUserByServiceID](_: any, data: IBasicID) {
      return (await UserService.fetchUserByServiceID(data));
    }
  },
  Mutation: {
    ...
    async [updateResidence](_: any, prop: GQLInputResidenceForm) {
      const response = await UserService.updateResidenceUser(prop)
      return response
    }
  },
}
