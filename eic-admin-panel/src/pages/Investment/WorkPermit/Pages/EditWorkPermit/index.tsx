...
import { Selectors as WorkPermitSelectors } from "src/store/States/Investment/WorkPermit"
...
const WorkPermit = () => {
  ...
  const selectedWorkPermitSelectors = useSelector(WorkPermitSelectors.selectSelectedWorkPermit);
  const [mainData, setMainData] = useState({ addedProducts: [] });
  const [tabsData, setTabsData] = useState(1);

  useEffect(() => {
    setMainData(selectedWorkPermitSelectors)
  }, [selectedWorkPermitSelectors, setMainData])
