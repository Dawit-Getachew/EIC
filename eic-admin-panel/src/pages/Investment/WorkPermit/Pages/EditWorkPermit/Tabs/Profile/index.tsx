...
const ProjectProfile: React.FC<Props> = (props) => {
  ...
  const [formData, setFormData] = useState({
    category_sector: "", category_sub_sector: "", category_activity: "", category_investment_activity: ""
  });
  const [errorData, setErrorData] = useState({});

  const projects = useSelector(ProjectSelectors.selectProjects)
  useEffect(() => {
    if (props.mainData.project) {
      setFormData({
        ...props.mainData,
        ...getProject(props.mainData.project, projects),
        ...getProject(props.mainData.project, projects).address
      });
    }
  ...
  const getValue = (name: string) => {
    const foundIndex = Object.keys(formData).findIndex(
      (key) => key === name
    );
    return foundIndex >= 0 ? formData[name] : "";
  };

  useEffect(() => {
    if (formData?.category_sector) {
      setAllowedSubSectors(
        props.sub_sectors.filter(
          (sub_sector) => String(sub_sector.sector) === String(formData.category_sector)
        )
      );
    }
    if (formData?.category_sub_sector) {
      setAllowedActivities(
        props.activities.filter(
          (activity) => String(activity.sub_sector) === String(formData.category_sub_sector)
        )
      );
    }
    if (formData?.category_activity) {
      setAllowedInvestmentActivities(
        props.investment_activities.filter(
          (investment_activity) =>
            String(investment_activity.activity) === String(formData.category_activity)
        )
      );
    }
  }, [props, formData, setAllowedInvestmentActivities])

  return getValue("project_stage").length === 0? <></> : (
    <>
      <Typography
      ...
      >
        <Grid item md={4}>
          <TextField
            ...
            sx={{ width: 1 }}
            name="start_date"
            defaultValue={getCorrectDate(getValue("start_date"))}
        ...
        <Grid item md={4}>
          <TextField
            ...
            sx={{ width: 1 }}
            name="end_date"
            defaultValue={getCorrectDate(getValue("end_date"))}
      ...
      <Grid container spacing={3}>
        <Grid item md={6}>
          <TextField
            ...
            defaultValue={getValue("category_sector")}
            error={getError("category_sector").value}
            helperText={getError("category_sector").text}
            onChange={handleChange}
          >
        ...
        <Grid item md={6}>
          <TextField
            ...
            sx={{ width: 1 }}
            select
            name="category_sub_sector"
            defaultValue={getValue("category_sub_sector")}
            error={getError("category_sub_sector").value}
            helperText={getError("category_sub_sector").text}
            onChange={handleChange}
          >
        ...
        <Grid item md={6}>
          <TextField
            ...
            defaultValue={getValue("category_activity")}
            error={getError("category_activity").value}
            helperText={getError("category_activity").text}
            onChange={handleChange}
          >
        ...
        <Grid item md={6}>
          <TextField
            ...
            sx={{ width: 1 }}
            select
            name="category_investment_activity"
            defaultValue={getValue("category_investment_activity")}
            error={getError("category_investment_activity").value}
            helperText={getError("category_investment_activity").text}
            onChange={handleChange}
          >
