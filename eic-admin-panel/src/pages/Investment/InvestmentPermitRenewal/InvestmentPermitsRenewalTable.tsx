...
import { Actions as InvestmentPermitRenewalActions, API as InvestmentPermitRenewalAPI } from "src/store/States/Investment/InvestmentPermitRenewal"
...
const RecentOrdersTable: FC<RecentOrdersTableProps> = ({ cryptoOrders }) => {
  ...
  const UpdatePermit: FC<{ investmentPermit: any }> = ({ investmentPermit }) => {
    ...
    const updateStatus = (_id: string) => {
      setIsLoading(true)
      InvestmentPermitRenewalAPI.UpdateStatusInvestmentPermitRenewal(_id, getRenewPermitStatus(user_role), (err, data) => {
        if (err) throw err
        setIsLoading(false)
        if (data._id) {
          dispatch(InvestmentPermitRenewalActions.UpdateInvestmentPermitRenewals(data))
        }
      })
    }
    ...
    return (
      <Tooltip title={getActionText(user_role)} arrow>
        <IconButton
          ...
          disabled={investmentPermit.permit_status === getRenewPermitStatus(user_role)}
        >
            ...
            {filterRenewalPermits(user_role, paginatedInvestmentPermitRenewals).map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <TableCell align="right">
                    <Tooltip title="View Renew Permit" arrow>
                      <IconButton
                        sx={{
                          '&:hover': {
                            background: theme.colors.primary.lighter
                        ...
                        size="small"
                        onClick={() => {
                          dispatch(BufferActions.SetReNewPermitBuffer(investmentPermit))
                          navigate(routes.INVESTMENT.VIEW_INVESTMENT_PERMIT_RENEWAL.ROUTE, { replace: true })
                        }}
                      >
                    ...
                    {/* <Tooltip title="Delete Permit" arrow>
                      <IconButton
                        ...
                        size="small"
                        onClick={() => {
                          setSelectedInvestmentPermitRenewal(investmentPermit)
