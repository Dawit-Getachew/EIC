...
import { Actions as InvestmentPermitAmmendmentActions, API as InvestmentPermitAmmendmentAPI } from "src/store/States/Investment/InvestmentPermitAmmendment";
...
const RecentOrdersTable: FC<RecentOrdersTableProps> = ({ cryptoOrders }) => {
  ...
  const getPermitStatus = (user_role: string) => {
    switch (user_role) {
      case "TEAM_LEADER": {
        return "VERIFIED"
      }
  ...
  const UpdatePermit: FC<{ investmentPermit: any }> = ({ investmentPermit }) => {
    ...
    const updateStatus = (_id: string, investment_id: string) => {
      setIsLoading(true)
      InvestmentPermitAmmendmentAPI.UpdateStatusInvestmentPermitAmmendment(_id, getPermitStatus(user_role), (err, data) => {
        if (err) throw err
        setIsLoading(false)
        if (data._id) {
          dispatch(InvestmentPermitAmmendmentActions.UpdateInvestmentPermitAmmendments(data))
          if (data.permit_status === "APPROVED") {
            InvestmentPermitAPI.UpdateInvestmentPermit({
              ...data,
              _id: investment_id
            }, (err, data_l) => {
              if (err) throw err
              console.log("ll", data_l)
            })
          }
        }
      })
    ...
    return (
      <Tooltip title={getActionText(user_role)} arrow>
        <IconButton
          ...
          size="small"
          onClick={() => {
            updateStatus(investmentPermit._id, investmentPermit.investment_id);
          }}
          disabled={
            (investmentPermit.permit_status ===
            getPermitStatus(user_role)) || isLoading
          }
        >
    ...
    <Card>
      ...
      {!selectedBulkActions && (
        <CardHeader
          ...
          title="Recent Investment Permit Amendments"
        />
      )}
      <Divider />
      <TableContainer>
                    ...
                    <TableCell align="right">
                      <Tooltip title="View Ammendment Request" arrow>
                        <IconButton
                          ...
                          onClick={() => {
                            dispatch(
                              BufferActions.SetNewPermitBuffer(investmentPermit)
                            );
                            navigate(
                              routes.INVESTMENT.AMEND_INVESTMENT_PERMIT_VIEW
                      ...
                      <Tooltip title="View Permit" arrow>
                        <IconButton
                          ...
                          onClick={() => {
                            ...
                            navigate(
                              routes.INVESTMENT.VIEW_INVESTMENT_PERMIT.ROUTE + `/${investmentPermit._id}`,
                              { replace: true }
                            );
                          }}
                        >
