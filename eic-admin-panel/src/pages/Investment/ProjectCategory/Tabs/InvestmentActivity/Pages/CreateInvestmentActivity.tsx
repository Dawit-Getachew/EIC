...
import { TextField, Grid, MenuItem, Container, CircularProgress } from "@mui/material/";
import {
...
import { API } from "src/store/States/Investment/Category/InvestmentActivity/";
import { Actions as BufferActions } from "src/store/States/Buffer"
import { useDispatch, useSelector } from "react-redux"

const CreateInvestmentActivity: React.FC<any> = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = (input: IInvestmentActivityInput) => {
    setIsLoading(true);
    API.CreateInvestmentActivity(input, (err, data) => {
      if (err) throw err;
      setIsLoading(false);
      if (data._id) {
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              <Card sx={{ p: 4 }}>
                <Typography
                  ...
                  sx={{ mb: 4 }}
                >
                  InvestmentActivity Information
                </Typography>
                <Grid
                  container
                  spacing={3}
...
export default CreateInvestmentActivity;