...
const DeleteModal: React.FC<DeleteModalProps> = ({ isVisible, onClose, business_profile }) => {
  const [isLoading, setIsLoading] = useState(false)
  const dispatch = useDispatch()
  const yesHandler = () => {
    setIsLoading(true)
    API.RemoveBusinessProfile(business_profile._id, (err, data) => {
      if (err) throw err
      setIsLoading(false)
      if (data._id) {
        dispatch(Actions.RemoveBusinessProfile(data))
        onClose()
      }
    })
  }
      ...
      >
        <Box sx={style}>
          <Typography id="modal-modal-title" variant="h4" component="h2" style={{ marginBottom: 15, marginTop: 5 }}>
            Are you sure you want to delete {business_profile.company_name}
            <Grid
              container
              direction="row"
