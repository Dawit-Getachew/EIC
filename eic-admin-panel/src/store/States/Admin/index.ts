...
import {
  ...
  RemoveAdminBody, RemoveAdminBodyTag
} from './query'
...
export const API = {
  ...
  RemoveAdmin: (_id: string, callback: Function) => {
    Axios.post(endPoints.baseURL, RemoveAdminBody(_id))
      .then(res => {
        if (res.status === 200) {
          callback(null, res.data.data[RemoveAdminBodyTag])
        } else {
          callback(res, null)
        }
      })
...
export const reducer = (state: any = initialState, action: any) => {
  switch (action.type) {
    ...
    case constants.REMOVE_ADMIN: {
      return {
        ...state, admins: state.admins.filter((item: any) => action.payload._id !== item._id)
      }
    }
...
export const Actions = {
  ...
  RemoveAdmin: (payload: any) => ({ type: constants.REMOVE_ADMIN, payload }),
}
