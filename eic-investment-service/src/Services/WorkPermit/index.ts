...
import { ValidationError } from "Common/constants"
...
class WorkPermitService {
  ...
  static async postWorkPermit(prop: IPostWorkPermit) {
    const foundProject = await ProjectService.fetchOneProjectByID({ _id: prop.input.project })
    if (!foundProject._id) return foundProject

    const foundManager = await ManagerService.fetchOneManagerByID({ _id: prop.input.selected_manager })
    if (!foundManager._id) return foundManager

    const foundProjects = await ProductService.fetchManyProductsByID({ _ids: prop.input.products })
    if (foundProjects.length !== prop.input.products.length || prop.input.products.length === 0) return {
      __typename: "ValidationError",
      error_path: "products",
      errors: [{ error_code: 400, error_message: "Invalid product ID" }]
    } as ValidationError
  ...
  static async editWorkPermit(prop: IEditWorkPermit) {
    if (prop.input.project) {
      const foundProject = await ProjectService.fetchOneProjectByID({ _id: prop.input.project })
      if (!foundProject._id) return foundProject
    }

    if (prop.input.selected_manager) {
      const foundManager = await ManagerService.fetchOneManagerByID({ _id: prop.input.selected_manager })
      if (!foundManager._id) return foundManager
    }

    if (prop.input.products) {
      const foundProjects = await ProductService.fetchManyProductsByID({ _ids: prop.input.products })
      if (foundProjects.length !== prop.input.products.length || prop.input.products.length === 0) return {
        __typename: "ValidationError",
        error_path: "products",
